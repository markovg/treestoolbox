...
for ward = 1 : lenf
    ...
    if ~isempty (a),
        ...
        if ~isempty (a),
            ...
            P0 = [P0, zeros(1, 4 - length (P0))]; % fill up with zeroes if not full
            P {end} = [P{end}; P0]; PL {end} = [PL{end}; Plevel];
            R {end} = [R{end}; regions(ward)];
            file {ward} (a : b) = '';
            if length (PL {end}) > 1,
                if Zflag, % if tree was just closed open a new tree with the new coordinates
                    ...
                    if PL {end} (end) < PL {end} (end - 1)
                        ipar {end} (end + 1) = find (PL {end} (1 : end - 1) == ...
                            Plevel - 1, 1, 'last');
                        Tflag = 0;
                    else
        ...
        u1 = strfind (file {ward}, '('); u2 = strfind (file {ward}, ')');
        if ~isempty (u1),
            ...
            for tissimo = 1 : length (S),
                if (~strcmp (S {tissimo},'dA')) && (~strcmp (S {tissimo}, 'rnames')),
                    if strcmp (S {tissimo}, 'R'),
                        ...
                        tree {uispine (ward)}.(S {tissimo}) = ...
                            [tree{uispine (ward)}.(S {tissimo}); spines{indy (te)}.(S {tissimo})];
                    end
                end
            end
        end
...
for ward = 1 : length (tree)
    ...
    for te = 1 : length (mtree)
        Dist = sqrt ((mtree {te}.X - XR).^2 +...
            (mtree {te}.Y - YR).^2 + (mtree {te}.Z - ZR).^2) - ...
            mtree {te}.D./2;
        [iry indy] = min (Dist);
        if iry < IRY,
            INDY = indy;
